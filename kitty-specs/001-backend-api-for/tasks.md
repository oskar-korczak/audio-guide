...
### Risks & Mitigations
- Go version mismatch → specify in go.mod

---

## Work Package WP02: Core Audio Generation (Priority: P1) 🎯 MVP

**Goal**: Implement the complete audio generation pipeline - OpenAI facts → OpenAI script → ElevenLabs audio.
...
3. Handler orchestrates: validate → facts → script → audio → return MP3
...
### Risks & Mitigations
- API key not set → clear error message on startup
...
### Included Subtasks
- [ ] T011 Implement error mapping utility (API errors → user messages)
...
### Risks & Mitigations
- Cascading timeouts → ensure per-service timeouts sum to less than total
...
### Risks & Mitigations
- Cold start latency → acceptable per spec (up to 90s)
- API keys exposed in command history → use secrets manager or env file
...
### Risks & Mitigations
- CORS errors → verified in WP02
- Old cached code exposing keys → hard refresh / clear cache
